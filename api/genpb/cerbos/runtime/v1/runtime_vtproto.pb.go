...
func (m *IndexBuildErrors_MissingImport) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ImportingPolicy) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.ImportingPolicy)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Desc) > 0 {
...
func (m *IndexBuildErrors) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.NumDisabled != 0 {
		i = encodeVarint(dAtA, i, uint64(m.NumDisabled))
		i--
		dAtA[i] = 0x30
	}
...
func (m *IndexBuildErrors) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field NumDisabled", wireType)
			}
			m.NumDisabled = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.NumDisabled |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
