...
func (m *RuleTable_RuleRow) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.DerivedRoleConstants) > 0 {
		for k := range m.DerivedRoleConstants {
			v := m.DerivedRoleConstants[k]
			baseI := i
	...
	if len(m.DerivedRoleOrderedVariables) > 0 {
		for iNdEx := len(m.DerivedRoleOrderedVariables) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.DerivedRoleOrderedVariables[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *RuleTable_RuleRow) SizeVT() (n int) {
	...
	if len(m.DerivedRoleOrderedVariables) > 0 {
		for _, e := range m.DerivedRoleOrderedVariables {
			l = e.SizeVT()
			n += 2 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
	if len(m.DerivedRoleConstants) > 0 {
		for k, v := range m.DerivedRoleConstants {
			_ = k
			_ = v
			l = 0
			if v != nil {
...
func (m *RuleTable_RuleRow) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 18:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DerivedRoleOrderedVariables", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.DerivedRoleOrderedVariables = append(m.DerivedRoleOrderedVariables, &Variable{})
			if err := m.DerivedRoleOrderedVariables[len(m.DerivedRoleOrderedVariables)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 19:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field DerivedRoleConstants", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.DerivedRoleConstants == nil {
				m.DerivedRoleConstants = make(map[string]*structpb1.Value)
			}
			var mapkey string
			var mapvalue *structpb1.Value
			for iNdEx < postIndex {
			...
			m.DerivedRoleConstants[mapkey] = mapvalue
			iNdEx = postIndex
		case 20:
			if wireType != 2 {
