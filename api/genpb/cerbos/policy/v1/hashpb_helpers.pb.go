...
func cerbos_engine_v1_AuxData_JWT_hashpb_sum(m *v1.AuxData_JWT, hasher hash.Hash, ignore map[string]struct{}, b *[10]byte) {
	...
	if _, ok := ignore["cerbos.policy.v1.TestSuite.tests"]; !ok {
		if len(m.Tests) > 0 {
			for _, v := range m.Tests {
				if v != nil {
					cerbos_policy_v1_TestTable_hashpb_sum(v, hasher, ignore, b)
				}
			}
		}
	}
			...
			if len(m.Principals) <= 32 {
				...
				for _, k := range keys {
					...
					if m.Principals[k] != nil {
						cerbos_engine_v1_Principal_hashpb_sum(m.Principals[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
			...
			if len(m.Resources) <= 32 {
				...
				for _, k := range keys {
					...
					if m.Resources[k] != nil {
						cerbos_engine_v1_Resource_hashpb_sum(m.Resources[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
		...
		if len(m.AuxData) > 0 {
			if len(m.AuxData) <= 32 {
				...
				for _, k := range keys {
					...
					_, _ = hasher.Write(unsafe.Slice(unsafe.StringData(k), len(k)))
					if m.AuxData[k] != nil {
						cerbos_engine_v1_AuxData_hashpb_sum(m.AuxData[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
			...
			if len(m.PrincipalGroups) <= 32 {
				...
				for _, k := range keys {
					...
					if m.PrincipalGroups[k] != nil {
						cerbos_policy_v1_TestFixtureGroup_Principals_hashpb_sum(m.PrincipalGroups[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
			...
			if len(m.ResourceGroups) <= 32 {
				...
				for _, k := range keys {
					...
					if m.ResourceGroups[k] != nil {
						cerbos_policy_v1_TestFixtureGroup_Resources_hashpb_sum(m.ResourceGroups[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
