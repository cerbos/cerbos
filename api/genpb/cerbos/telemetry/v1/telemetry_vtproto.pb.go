...
func (m *ServerLaunch_Stats_Policy) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.MaxConditionCount) > 0 {
		for k := range m.MaxConditionCount {
			v := m.MaxConditionCount[k]
			baseI := i
			i = protohelpers.EncodeVarint(dAtA, i, uint64(v))
			i--
			dAtA[i] = 0x10
...
func (m *ServerLaunch_Stats_Policy) SizeVT() (n int) {
	...
	if len(m.ConditionCount) > 0 {
		for k, v := range m.ConditionCount {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + protohelpers.SizeOfVarint(uint64(len(k))) + 1 + protohelpers.SizeOfVarint(uint64(v))
			n += mapEntrySize + 1 + protohelpers.SizeOfVarint(uint64(mapEntrySize))
	...
	if len(m.MaxConditionCount) > 0 {
		for k, v := range m.MaxConditionCount {
			_ = k
			_ = v
			mapEntrySize := 1 + len(k) + protohelpers.SizeOfVarint(uint64(len(k))) + 1 + protohelpers.SizeOfVarint(uint64(v))
			n += mapEntrySize + 1 + protohelpers.SizeOfVarint(uint64(mapEntrySize))
...
func (m *ServerLaunch_Stats_Policy) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ConditionCount", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.ConditionCount == nil {
				m.ConditionCount = make(map[string]uint32)
			}
			var mapkey string
			var mapvalue uint32
			for iNdEx < postIndex {
			...
			m.ConditionCount[mapkey] = mapvalue
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
			...
			if m.RuleCount == nil {
				m.RuleCount = make(map[string]uint32)
			}
			var mapkey string
			var mapvalue uint32
			for iNdEx < postIndex {
			...
			m.RuleCount[mapkey] = mapvalue
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxConditionCount", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.MaxConditionCount == nil {
				m.MaxConditionCount = make(map[string]uint32)
			}
			var mapkey string
			var mapvalue uint32
			for iNdEx < postIndex {
			...
			m.MaxConditionCount[mapkey] = mapvalue
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxRuleCount", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.MaxRuleCount == nil {
				m.MaxRuleCount = make(map[string]uint32)
			}
			var mapkey string
			var mapvalue uint32
			for iNdEx < postIndex {
