...
func (m *SourceContext) SizeVT() (n int) {
	...
	if len(m.MapKeyPositions) > 0 {
		for k, v := range m.MapKeyPositions {
			_ = k
			_ = v
			l = 0
			if v != nil {
...
func (m *SourceContext) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MapKeyPositions", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.MapKeyPositions == nil {
				m.MapKeyPositions = make(map[string]*Position)
			}
			var mapkey string
			var mapvalue *Position
			for iNdEx < postIndex {
			...
			m.MapKeyPositions[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
