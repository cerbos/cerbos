...
func cerbos_source_v1_PolicyWrapper_hashpb_sum(m *PolicyWrapper, hasher hash.Hash, ignore map[string]struct{}, b *[10]byte) {
	if _, ok := ignore["cerbos.source.v1.PolicyWrapper.id"]; !ok {
		_, _ = hasher.Write(protowire.AppendVarint(b[:0], m.GetId()))
	}
	...
	if _, ok := ignore["cerbos.source.v1.PolicyWrapper.policy"]; !ok {
		if m.GetPolicy() != nil {
			cerbos_policy_v1_Policy_hashpb_sum(m.GetPolicy(), hasher, ignore, b)
		}
	}
...
func cerbos_source_v1_SourceContext_hashpb_sum(m *SourceContext, hasher hash.Hash, ignore map[string]struct{}, b *[10]byte) {
	if _, ok := ignore["cerbos.source.v1.SourceContext.start_position"]; !ok {
		if m.GetStartPosition() != nil {
			cerbos_source_v1_StartPosition_hashpb_sum(m.GetStartPosition(), hasher, ignore, b)
		}
	}
	if _, ok := ignore["cerbos.source.v1.SourceContext.field_positions"]; !ok {
		if len(m.FieldPositions) > 0 {
			if len(m.FieldPositions) <= 32 {
				keys := hashpb_stringKeyPool.Get().([]string)[:0]
				...
				for _, k := range keys {
					...
					if m.FieldPositions[k] != nil {
						cerbos_source_v1_Position_hashpb_sum(m.FieldPositions[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
	...
	if _, ok := ignore["cerbos.source.v1.SourceContext.map_key_positions"]; !ok {
		if len(m.MapKeyPositions) > 0 {
			if len(m.MapKeyPositions) <= 32 {
				keys := hashpb_stringKeyPool.Get().([]string)[:0]
				for k := range m.MapKeyPositions {
					keys = append(keys, k)
				}
				slices.Sort(keys)
				for _, k := range keys {
					...
					if m.MapKeyPositions[k] != nil {
						cerbos_source_v1_Position_hashpb_sum(m.MapKeyPositions[k], hasher, ignore, b)
					}
				}
				hashpb_stringKeyPool.Put(keys)
			} else {
...
func cerbos_source_v1_StartPosition_hashpb_sum(m *StartPosition, hasher hash.Hash, ignore map[string]struct{}, b *[10]byte) {
	...
	if _, ok := ignore["cerbos.source.v1.StartPosition.offset"]; !ok {
		_, _ = hasher.Write(protowire.AppendVarint(b[:0], uint64(m.GetOffset())))
	}
}
