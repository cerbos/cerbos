...
----
# TODO: Replace with link to Docker registry
make container DOCKER_IMAGE={docker_image}
----
...
Let's start the server and try out a few requests.

[source,shell,subs="attributes"]
----
docker run -it -v $(pwd):/photo-share -p 9999:9999 {docker_image} server --config=/photo-share/config.yaml
----
...
.Bradley trying to view Alicia's private album
[source,shell,linenums,highlight=2..18]
----
cat <<EOF | curl --silent "localhost:9999/v1/check" -d @-
{
  "requestId":  "test02",
...
.Bradley trying to view Alicia's public album
[source,shell,linenums,highlight=2..18]
----
cat <<EOF | curl --silent "localhost:9999/v1/check" -d @-
{
...
.Maria, a moderator, trying to delete an album that is _not_ flagged as inappropriate
[source,shell,linenums,highlight=2..18]
----
cat <<EOF | curl --silent "localhost:9999/v1/check" -d @- 
{
...
----
{"requestId":"test04","statusCode":401,"statusMessage":"Deny","effect":"EFFECT_DENY"}
----


.Maria, a moderator, trying to delete an album that is flagged as inappropriate
[source,shell,linenums,highlight=2..18]
----
cat <<EOF | curl --silent "localhost:9999/v1/check" -d @- 
{
...
----
{"requestId":"test05","statusCode":200,"statusMessage":"Allow","effect":"EFFECT_ALLOW"}
----
