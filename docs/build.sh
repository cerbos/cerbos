...
ANTORA_VERSION=${ANTORA_VERSION:-"3.1.14"}

rm -rf "${SCRIPT_DIR}/build"
docker run --platform linux/amd64 \
    -v "$SOURCE_DIR":"${WORKSPACE}/cerbos":Z \
    --rm -t "docker.io/antora/antora:${ANTORA_VERSION}" \
    sh -c "yarn global add @cerbos/antora-llm-generator @antora/collector-extension && antora --stacktrace --clean ${WORKSPACE}/cerbos/docs/antora-playbook.yml"
