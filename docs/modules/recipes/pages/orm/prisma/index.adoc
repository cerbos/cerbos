...
https://prisma.io[Prisma]  is a powerful ORM for modern Node.js applications, and Cerbos provides a robust authorization layer that simplifies role-based access control (RBAC) and attribute-based access control (ABAC). With the latest Cerbos Prisma Query Plan Adapter, integrating dynamic authorization rules into your Prisma queries is easier than ever.
