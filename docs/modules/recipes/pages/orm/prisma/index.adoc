...
https://prisma.io[Prisma] is a powerful ORM for modern Node.js applications. The Cerbos Prisma query plan adapter provides a powerful integration that effortlessly converts Cerbos xref:api:index.adoc#resources-query-plan[query plan] responses into Prisma queries.
