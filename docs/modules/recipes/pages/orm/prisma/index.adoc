...
Having now set up our Prisma database, it is time to implement our web server. For this example we will be using Express to set up a simple server running on port 3000. We will also import our Prisma and Cerbos clients which we will use later on.
...
First, create a `cerbos` directory (https://github.com/cerbos/express-prisma-cerbos/tree/main/cerbos[see repo]) with the subdirectories; `config` and `policies`. Then create a file `config.yaml` inside the `cerbos` subdirectory. The contents of the config will tell the Cerbos instance which port to run on and where the policies are located. Run the following to create the files, and populate the config:

[source,bash]
----
mkdir -p cerbos/config cerbos/policies && cat << EOF > cerbos/config/config.yaml
---
server:
  httpListenAddr: ":3592"
storage:
...
Now that our server is set up, it is time to define our resource policy as per the requirements, which as a reminder, were:
...
Run the following to create the file, and populate it with the policy:
