...
----
{
  ...
  "containerDefinitions": [
    {
      ...
      "environment": [
        ...
        },
        {
          ...
          "value": "YOUR_DEPLOYMENT_ID" <3>
        }
      ],
      "command": [
        "server",
        "--set=audit.enabled=true", <4>
  ...
  "taskRoleArn": "TASK_ROLE_ARN", <5>
  "executionRoleArn": "TASK_EXECUTION_ROLE_ARN", <6>
  "networkMode": "awsvpc",
...
<2> Client secret from the Cerbos Hub credential
<3> Cerbos Hub xref:cerbos-hub:ROOT:deployments.adoc[Deployment ID]
<4> Enables audit log collection. See xref:cerbos-hub:ROOT:audit-log-collection.adoc[Hub audit log collection documentation] for information about masking sensitive fields and other advanced settings.
<5> The ARN for the custom ECS Task Role defined in Step 1.
<6> The ARN for the ECS Task Execution. The default is `arn:aws:iam::<AWS_ACCOUNT_ID>:role/ecsTaskExecutionRole`
