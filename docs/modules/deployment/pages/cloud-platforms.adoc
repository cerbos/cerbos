...
After launching Cerbos with LiteFS, it is possible to add, disable, list or update policies by using
link:https://docs.cerbos.dev/cerbos/latest/cli/cerbosctl.html#put[`cerbosctl`].
...
cerbosctl \
  ...
  --username="cerbos" \
  --password="cerbosAdmin" \
  get export_variables

cerbosctl \
  --server=https://<APPLICATION_NAME>.fly.dev \ <1>
  --username="cerbos" \
  --password="cerbosAdmin" \
  get principal_policies
