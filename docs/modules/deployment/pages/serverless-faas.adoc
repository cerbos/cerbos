...
----
<1> This must be a hard link to the `cerbosfunc` binary with matchinig CPU architecture
...
The layer then can be attached to your AWS Lambda function, if it is deployed as a Zip package.
...
----
<1> - the layer is a directory containing `cerbosext` binary
...
The Cerbos function/extension can be configured with the environment variables.
[source,yaml,linenum]
----
    ...
    CERBOS_CONFIG: "..." <1>
----
<1> - The Lambda function file at `/<filename>` available to the extension by `/var/task/<filename>` path.

The extension can be configured without the config file to work with Cerbos Hub.
[source,yaml,linenum]
----
