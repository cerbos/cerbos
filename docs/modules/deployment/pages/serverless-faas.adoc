...
Download the appropriate `cerbos_aws_lambda_func` tarball from the link:{app-github-releases-page}[releases page]. It contains a specially-built binary named `cerbosfunc`  that can be deployed as either a Zip package or a container image.

Deploying as a zip package requires renaming the `cerbosfunc` binary to `bootstrap` and creating a zip file with it at the root. See link:https://docs.aws.amazon.com/lambda/latest/dg/golang-package.html[Deploy Go Lambda functions with .zip file archives] for more information.
