...
storage:
  ...
  overlay: 
    baseDriver: postgres
    fallbackDriver: disk
    fallbackErrorThreshold: 1 # number of errors that occur within the fallbackErrorWindow to trigger failover
