...
audit:
  ...
  local:
    ...
    advanced:
      ...
      badgerMemTableSize: 33554432 # Size of the Badger memtable in bytes. Larger memtables buffer more writes in memory before flushing to disk, at the cost of a higher memory footprint and a larger write-ahead log file. Do not reduce this on an existing database after an unclean shutdown: the leftover write-ahead log (*.mem) must fit the new size to be replayed, and startup fails if it does not.
----
