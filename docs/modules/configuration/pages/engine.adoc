...
NOTE: This setting has no effect when <<strict_evaluation>> is enabled because evaluation stops at the first error, which is always logged at `error` level.
...
Setting `strictEvaluation` to `true` makes such errors fatal, so on an error the whole request is terminated. The response returns a generic `FAILED_PRECONDITION` error. The strict evaluation mode applies to both the check and the query planner APIs.
