...
* An error in the condition of an imported derived role denies the actions of the rules that reference the derived role in their `derivedRoles`, and the actions whose conditions access `runtime.effectiveDerivedRoles`. Actions that reference neither are not affected.
