...
Setting `strictEvaluation` to `true` makes such errors deny the affected action. This applies to both the check and the query planner APIs. The errors are logged (see <<cel_error_log_level>>) and included in the audit log entry as usual.
