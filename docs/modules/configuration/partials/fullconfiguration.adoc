...
engine:
  celErrorLogLevel: warn # CELErrorLogLevel is the level at which CEL runtime errors raised during policy evaluation are logged. Valid values are none, debug, info, warn (default) and error. This option is ignored if strictEvaluation mode is enabled.
