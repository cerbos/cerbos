...
engine:
  celErrorLogLevel: warn # CELErrorLogLevel is the level at which CEL runtime errors raised during policy evaluation are logged. Valid values are none, debug, info, warn (default) and error. This option has no effect if engine's strictEvaluation mode is enabled.
  ...
  strictEvaluation: false # StrictEvaluation mode, when enabled, terminates policy evaluation on any error.
hub:
