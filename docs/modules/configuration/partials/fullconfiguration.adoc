audit:
  ...
  hub:
    advanced: 
      badgerMemTableSize: 33554432 # Size of the Badger memtable in bytes. Do not reduce it on an existing database after an unclean shutdown: the leftover write-ahead log (*.mem) must fit the new size to be replayed, and startup fails if it does not.
      bufferSize: 256 
      flushInterval: 1s 
      gcInterval: 60s 
      maxBatchSize: 32 
...
engine:
  celErrorLogLevel: warn # CELErrorLogLevel is the level at which CEL runtime errors raised during policy evaluation are logged. Valid values are none, debug, info, warn (default) and error.
  ...
  strictEvaluation: false # StrictEvaluation denies the affected action when an error is raised during policy evaluation, instead of ignoring the erroring rule.
hub:
...
storage:
  ...
  hub:
    ...
    local: # Local holds configuration for local bundle source.
      ...
      encryptionKey: 9b941a7f43fcade02d1e07bdaca008aedd0310e8804bc465bc44814a2010ecd3 # EncryptionKey is encryption key to decode the bundle. It must be hex encoded.
    ...
    remote: # Remote holds configuration for remote bundle source. Takes precedence over local if both are defined.
      ...
      deploymentID: TVWD7S5W4V5O # DeploymentID to fetch from the server. Mutually exclusive with PlaygroundID
      disableAutoUpdate: false # DisableAutoUpdate sets whether new bundles should be automatically downloaded and applied.
      playgroundID: HDUDDWLR6ZVM # PlaygroundID to fetch from the server. Mutually exclusive with DeploymentID.
      tempDir: ${TEMP} # TempDir is the directory to use for temporary files.
  mysql:
