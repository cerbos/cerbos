...
storage:
  ...
  overlay:
    ...
    baseDriver: disk # Required. BaseDriver is the default storage driver
    failoverInterval: 5m # FailoverInterval is the cyclic period within which we aggregate failures
    failoverThreshold: 5 # FailoverThreshold is the max number of errors we allow within the failoverInterval period
    fallbackDriver: git # Required. FallbackDriver is the secondary or fallback storage driver
  postgres:
