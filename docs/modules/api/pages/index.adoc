...
Cerbos implements the link:https://openid.github.io/authzen/[OpenID AuthZEN Authorization API specification], providing a standardized way to make authorization decisions. The AuthZEN API uses a simplified entity model that maps to Cerbos's native authorization model.
...
- `subject.id` - Maps to `principal.id`
- `subject.properties` - Maps to `principal.attr` with special handling for Cerbos-specific fields:
  * `cerbos.roles` - Maps to `principal.roles` (array of role names)
  * `cerbos.policyVersion` - Maps to `principal.policyVersion`
  * `cerbos.scope` - Maps to `principal.scope`
  * All other properties are mapped to `principal.attr`

**Resource → Resource**::
- `resource.type` - Maps to `resource.kind`
- `resource.id` - Maps to `resource.id`
- `resource.properties` - Maps to `resource.attr` with special handling for Cerbos-specific fields:
  * `cerbos.policyVersion` - Maps to `resource.policyVersion`
  * `cerbos.scope` - Maps to `resource.scope`
  * All other properties are mapped to `resource.attr`

**Action → Actions**::
- `action.name` - Maps to an entry in the `actions` array
- `action.properties` - Currently unused but reserved for future extensions

**Context**::
The `context` field contains environmental/contextual data for policy evaluation and Cerbos-specific control fields:
  * `cerbos.requestId` - Maps to `requestId`
  * `cerbos.auxData` - Maps to `auxData` (for JWT tokens, etc.)
  * `cerbos.includeMeta` - Controls whether metadata is included in the response
...
----
{
  "policy_decision_point": "Cerbos", <1>
  "access_evaluation_endpoint": "/access/v1/evaluation", <2>
  "access_evaluations_endpoint": "/access/v1/evaluations" <3>
}
----
<1> Name of the Policy Decision Point implementation
<2> URL path for single access evaluation requests
<3> URL path for batch access evaluation requests


[#authzen-access-evaluation]
=== `AccessEvaluation` (`/access/v1/evaluation`)
...
=== `AccessEvaluationBatch` (`/access/v1/evaluations`)
