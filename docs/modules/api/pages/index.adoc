...
<3> Resource policy version. Optional. The server falls back to the xref:configuration:engine.adoc[configured default version] if this is not specified.
<4> Resource kind. Required. This value is used to determine the resource policy to evaluate.
<5> Free-form context data about this resource instance. Policy rule conditions are evaluated based on these values.
<6> ID of the principal performing the actions. Required.
...
<8> Static roles that are assigned to this principal by your identity management system. Required.
<9> Free-form context data about this principal. Policy rule conditions are evaluated based on these values.
<10> An optional flag to signal that the response should include metadata about evaluation. Useful for debugging.
<11> Optional section for providing auxiliary data.
<12> JWT to use as an auxiliary data source.
<13> ID of the keyset to use to verify the JWT. Optional if only a single xref:configuration:auxdata.adoc[keyset is configured].
...
----
<1> Abstract syntax tree (AST) of the policies' expressions, which have been partially evaluated using values provided in the request.
