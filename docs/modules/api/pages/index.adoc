...
* link:https://www.cerbos.dev/workshop?utm_campaign=brand_cerbos&utm_source=documentation&utm_medium=text&utm_content=api_index[Talk to an engineer about API integration or query plans]
...
<15> Single JWT to use as an auxiliary data source. Mutually exclusive with `jwts`.
<16> Encoded token string.
...
<18> Multiple JWTs to use as auxiliary data. Since Cerbos PDP 0.55.0 only. Mutually exclusive with `jwt`.
<19> Named JWT.
<20> Encoded token string.
<21> ID of the keyset to use to verify the JWT. Optional if only a single xref:configuration:auxdata.adoc[keyset is configured].
<22> Optional flag to receive metadata about request evaluation.
...
<16> Encoded token string.
<17> ID of the keyset to use to verify the JWT. Optional if only a single xref:configuration:auxdata.adoc[keyset is configured].
<18> Multiple named JWTs. Since Cerbos 0.55.0 only. Mutually exclusive with `jwt`.
<19> Named JWT.
<20> Encoded token string.
<21> ID of the keyset to use to verify the JWT. Optional if only a single xref:configuration:auxdata.adoc[keyset is configured].
