...
Use `nameRegexp` and `scopeRegexp` to filter using the policy name and scope with case insensitive regular expression. Use `version` to filter using the full policy version string.
