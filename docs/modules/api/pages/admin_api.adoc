...
PUT    /admin/policy/delete?id=policy_id
----
...
Issue a DELETE request to the endpoint to delete all rows in the `policy_revisions` table.

Use `keepLast=N` query parameter to keep the last N revisions. If not specified all revisions are going to be deleted.
