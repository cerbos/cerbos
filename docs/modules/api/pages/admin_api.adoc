...
----
GET /admin/policies/metadata
----

Issue a GET request to the endpoint to list the metadata of the policies available in the store. If the policy store
supports filtering, you can optionally pass filter parameters to reduce the result set.
...
----
curl -k -u cerbos:cerbosAdmin \
    'https://localhost:3592/admin/policies/metadata'
----

[source,shell]
----
curl -k -u cerbos:cerbosAdmin \
    'https://localhost:3592/admin/policies/metadata?includeDisabled=true&nameRegexp=%5Efoo&scopeRegexp=bar%24&versionRegexp=default'
----
