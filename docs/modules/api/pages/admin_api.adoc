...
== Add/update schemas [`/admin/schema`]
...
<2> base64 encoded schema definition
...
== List schemas [`/admin/schemas`]

Issue a GET request to the endpoint to list the schemas available in the store.

NOTE: Only the schema ids will be responded for this request. Please use the get schema endpoint to retrieve related
schema's definition.
...
== Get schema(s) [`/admin/schema?id=principal.json&id=leave_request.json`]
...
== Delete schema(s) [`/admin/schema?id=principal.json&id=leave_request.json`]
