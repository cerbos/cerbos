...
Issue a GET request to the endpoint to list the policies available in the store. You can optionally pass filter parameters to reduce the result set.

Use `includeDisabled=true` query parameter in order to include disabled policies in the response. `nameRegexp` and `scopeRegexp` both support regular expression. `version` requires a full string match.
...
----
curl -k -u cerbos:cerbosAdmin \
    'https://localhost:3592/admin/policies?pretty&includeDisabled=true&nameRegexp=%5Efoo&scopeRegexp=bar%24&version=default'
----
