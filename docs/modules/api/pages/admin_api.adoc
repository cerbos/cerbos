...
Use `nameRegexp` and `scopeRegexp` to filter using the policy name and scope with regular expression. Use `version` to filter using the full policy version string.
