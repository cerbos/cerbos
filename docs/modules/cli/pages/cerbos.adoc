...
`compile-store`:: Validate and compile policies on any Cerbos store
...
[source]
----
Usage: cerbos compile <dir> [flags]
...
Flags:
  -h, --help                       Show context-sensitive help.
      ...
      --test-filter=TEST-FILTER    Filter tests by dimensions (suite, test, principal, resource, action). Format: 'dimension=glob1,glob2;...'. Can be specified multiple times.
      --skip-tests                 Skip tests
      --skip-batching              Skip batching tests
      --strict-evaluation          Run tests with strict evaluation enabled.
...
[#compile-store]
== `compile-store` Command

Runs the Cerbos compiler to validate policy definitions on any Cerbos store. See xref:policies:compile.adoc[Policy compilation] and xref:configuration:storage.adoc[Storage] for more information.
...
[source]
----
Usage: cerbos compile-store <config> [flags]
...
Arguments:
  <config>    Path to config file ($CERBOS_CONFIG)
...
Flags:
  -h, --help                                    Show context-sensitive help.
      ...
      --disable-invalid                         Disable invalid policies if database store
      --assume-yes                              Answer yes to all confirmation questions
  -o, --output="tree"                           Output format (tree,list,json)
      --color=COLOR                             Output color level (auto,never,always,256,16m). Defaults to auto.
      --no-color                                Disable colored output
----
