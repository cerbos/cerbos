...
You can filter the output using the `kind`, `name` and `version` flags. Each flag accepts multiple comma-separated values which are OR'ed together. For example, `--kind=resource,principal` matches policies that are either resource policies or principal policies.

Possible values for `kind` are `derived_roles`, `resource` and `principal`.
...
.List policies where `kind` is `derived_roles` or `resource`
----
cerbosctl list --kind derived_roles,resource
----

.List policies where `name` is `my_policy` or `a_policy`
----
cerbosctl list --version my_policy,a_policy
----

.List policies where `version` is `default` or `v1`
----
cerbosctl list --version default,v1
----

.List YAML
----
cerbosctl list --format=yaml
----

== `get`
This command gets the policies available in the store by their policy key (`<kind>.<name>.<version>`).

.Get summary
----
cerbosctl get derived_roles.my_derived_roles principal.donald_duck.default
----
