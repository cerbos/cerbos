...
This command lists the policies stored in the Cerbos server. It accepts the filters below to enable filtering the list. The filters use JSONPath expressions to query fields.
See link:https://tools.ietf.org/id/draft-goessner-dispatch-jsonpath-00.html#table-1[specification] for detailed information about JSONPath.

[#list-filters]
.Filters
****

--field-eq:: Filter a field with an exact match
+ 
- `--field-eq="$.resourcePolicy.version=20210210"`: Will list resource policies with version 20210210.
- `--field-eq="$.*.version=20210210"`: Will list any type of policies with version 20210210.
--field-match:: Filter a field with a regex match. 
+
- `--field-match="$.resourcePolicy.resource=leave"`: Will list the resource policies with "leave" contained in the resource field.
- `--field-match="$.resourcePolicy.version=\d"`: Will list resource policies with a version includes a numeric value.
...
This command displays the detailed version information of the cerbosctl and the Cerbos server it connected.
