...
Hidden directories are now ignored by the disk-based storage backends like `disk`, `git` and `blob` in order to ignore system metadata directories and other control files. Furthermore, schema IDs are now based on the path of the schema file relative to the `_schema` directory instead of the file basename.   
