...
By default, Prometheus metrics are available from the `/_cerbos/metrics` HTTP endpoint. If you wish to disable metrics reporting, set `metricsEnabled` to `false`.
...
When <<intro-cerbos-policies,authoring Cerbos policies>> you can optionally specify a policy `version`. The purpose of the `version` field is to support use cases such as having different policies for different environments (production, staging etc.) or for gradual rollout of a new version of an application. By default, when a request does not explicitly specify the policy version, the Cerbos engine attempts to find a matching policy that has its version set to `default`. You can change the fallback version by setting the `defaultPolicyVersion`. For example, if you have a Cerbos deployment for your staging environment, you may want to set `defaultPolicyVersion: staging` to ensure that the default policies in effect are the ones versioned as `staging`.


== Tracing block

Cerbos supports distributed tracing. To enable tracing, set `sampleProbability` to a value between 0.0 and 1.0. Setting the probability to 1.0 makes Cerbos capture tracing information for all requests and setting it to 0.0 disables capturing any traces. 
...
The storage section defines how Cerbos accesses the set of access policies to make authorization decisions. The `driver` setting is used to determine the storage driver to use. Currently the supported driver values are `disk` and `git`.
