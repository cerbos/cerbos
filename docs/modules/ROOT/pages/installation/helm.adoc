...
Add the Cerbos Helm repository using the username and API key provided to you:

[source,sh,subs="attributes,macros"]
----
helm repo add cerbos https://cerbos.jfrog.io/artifactory/charts --username $$<$$YOUR_USERNAME$$>$$ --password $$<$$YOUR_API_KEY$$>$$
helm repo update
----
...
[source,sh,subs="attributes,macros"]
----
kubectl create secret docker-registry cerbos-registry --docker-server={app-container-registry} --docker-username=$$<$$YOUR_USERNAME$$>$$ --docker-password=$$<$$YOUR_API_KEY$$>$$
----
