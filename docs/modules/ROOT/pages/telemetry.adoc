...
We use link:https://segment.com[Segment] to collect the data. Only a small number of Zenauth (the company leading the development of Cerbos) employees have access to the data. 
...
docker run -i -t -p 3592:3592 \
    {app-docker-img} \
    server --config=/conf.default.yaml --set=telemetry.disabled=true
----
