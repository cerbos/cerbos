...
Cerbos is typically deployed as a standalone service or a sidecar. The the API can be accessed using REST or gRPC. For added security, Cerbos can serve both APIs on Unix domain sockets as well. The server can be configured to watch a git repository containing Cerbos policies and automatically update itself as new commits are added. 

.Cerbos workflow
* Author <<intro-cerbos-policies, Cerbos policies>>.
* Compile and validate the policies using the Cerbos CLI.
* Submit a pull-request to the policy repo. The policy repo can a central git repository shared by everybody in your organization or a subdirectory in your application itself.
* Optionaly, let your CI system run the Cerbos compilation stage automatically.
* Follow your process to push the change to "production".
* Cerbos server will automatically pull the latest commit from the production branch and update itself. Subsequent requests to the server will use the latest version of the policies.


[[intro-cerbos-policies]]
== Cerbos Policies
...
A typical REST request to the Cerbos API resembles the following.
