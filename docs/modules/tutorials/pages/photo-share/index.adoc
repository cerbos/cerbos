...
Update `config.yaml` to enable schema enforcement.
...
Now start the server again and send a request that does not conform to the schema. The server response should contain a list validation errors.
