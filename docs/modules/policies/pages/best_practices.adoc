...
Representing this a resource policy could be achieved in a variety of ways. Let's take a look at each:

Action-led:: Here, we focus on an action, and list all the roles that can perform that action:
...
Role-led:: Or we can focus on a role, and list all the actions the role can perform:
...
Hybrid:: Perhaps we want to use a combination of the two:
...
Blanket allow, granular deny:: Or perhaps we have the case where a principal can perform _nearly_ every action:
