...
NOTE: This precedence applies to rules matched within a single role evaluation. When a principal has multiple roles, each role is evaluated independently. If _any_ of the principal's roles produces an `EFFECT_ALLOW` for an action, access is granted — even if another role is denied or has no matching rule. In other words, deny-takes-precedence resolves conflicts between rules that match for the same role, not across different roles.
