...
| getMonth | Get month from a timestamp. Returns a zero-based value where January is 0 | timestamp(R.attr.lastAccessed).getMonth("Australia/Melbourne") == 3
...
NOTE: All timestamp getters—getHours, getMinutes, getDayOfWeek, and similar—take a timezone parameter. If omitted, the 'UTC' timezone is used by default.
