...
NOTE: The `inIPAddrRange` is a Cerbos-specific extension function to CEL.
...
|===
| Function | Description | Example
| cidr(<cidr>).containsCIDR | Returns whether the CIDR contains the given CIDR. Since v0.55.0 only. | cidr("192.168.0.0/8").containsCIDR(cidr("192.168.1.0/24")) == true
| cidr(<cidr>).containsIP | Returns whether the CIDR contains the given IP. Since v0.55.0 only. Use `inIPAddrRange` for older versions. | cidr("2001:db8::/48").containsIP(P.attr.ipv6Address) == true
| cidr(<cidr>).isMask | Returns whether the CIDR is a mask. Since v0.55.0 only. | cidr("192.168.0.0/8").isMask() == true
| cidr(<cidr>).masked | Returns the masked CIDR for the value. Since v0.55.0 only. | cidr("192.168.1.5/24").masked() == cidr("192.168.1.0/24")
| cidr(<cidr>).prefixLength | Returns the prefix length of the CIDR. Since v0.55.0 only. | cidr("192.168.1.0/24").prefixLength() == 24
...
| ip(<ip>).family | Returns the IP family. Since v0.55.0 only. | ip(P.attr.ipv4Address) == 4
| ip(<ip>).isGlobalUnicast | Returns true if the IP address is a global unicast address. Since v0.55.0 only. | ip(P.attr.ipv6Address).isGlobalUnicast() == true
| ip(<ip>).isLinkLocalMulticast | Returns true if the IP address is a link local multicast address. Since v0.55.0 only. | ip(P.attr.ipv6Address).isLinkLocalMulticast() == false
| ip(<ip>).isLinkLocalUnicast | Returns true if the IP address is a link local unicast address. Since v0.55.0 only. | ip(P.attr.ipv6Address).isLinkLocalUnicast() == false
| ip(<ip>).isLoopback | Returns true if the IP address is a loopback address. Since v0.55.0 only. | ip(P.attr.ipv4Address).isLoopback() == false
| ip(<ip>).isUnspecified | Returns true if the IP address is unspecified (either `0.0.0.0` or `::`). Since v0.55.0 only. | ip(P.attr.ipv6Address).isUnspecified() == false
| ip.isCanonical | Check whether the IPv6 string conforms to https://www.rfc-editor.org/rfc/rfc5952.html[RFC 5952]. Since v0.55.0 only. | ip.isCanonical(P.attr.ipv6Address) == false
| isCIDR | Returns true if the string is an IPv4 or IPv6 CIDR. Since v0.55.0 only. | isCIDR("::1/128") == true
| isIP | Returns true if the string is an IPv4 or IPv6 address. Since v0.55.0 only. | isIP(P.attr.ipv4Address) == true
|===
...
| sets.contains | Checks whether the first list contains all the elements of the second list. Since v0.55.0 only. | sets.contains(P.attr.teams, ["design"]) == true
| sets.equivalent | Checks whether for every item in the first list, there exists an equal item in the second list. Since v0.55.0 only. | sets.equivalent(P.attr.teams, ["commercial", "product", "communications", "design"])  == true
| sets.intersects | Checks whether the lists have at least one item in common. Since v0.55.0 only. Use `hasIntersection` for older versions. |  sets.intersects(["design", "engineering"], P.attr.teams) == true
...
| json.encode | Encode as JSON. Since v0.55.0 only. | json.encode(R.attr) == '{"id":"125","department":"marketing"}'
...
| regex.extractAll | Extract all occurrences of a https://github.com/google/re2/wiki/Syntax[RE2] regular expression match. Since v0.55.0 only. | regex.extractAll("a bb a cc", "a") == ["a", "a"]
| regex.replace | Replace all occurrences of a https://github.com/google/re2/wiki/Syntax[RE2] regular expression match. Since v0.55.0 only. | regex.replace("banana", "a", "i") == "binini"
| regex.replace | Replace occurrences of a https://github.com/google/re2/wiki/Syntax[RE2] regular expression match with a limit. Since v0.55.0 only. | regex.replace("banana", "a", "i", 1) == "binana" && regex.replace("banana", "b(a).*", r"\1") == "a"
...
[TIP]
====
For patterns that translate condition logic into database queries, see https://www.cerbos.dev/blog/filtering-database-results-with-cerbos-query-plans[Filtering database results with Cerbos query plans]. For implementation review, https://www.cerbos.dev/workshop?utm_campaign=brand_cerbos&utm_source=documentation&utm_medium=text&utm_content=conditions[speak to us].
====
