...
`request`:: The principal, resource, and auxiliary data being used to evaluate the policy.
`variables`:: Variables declared in the xref:variables.adoc[`variables` section of the policy].
...
<4> xref:derived_roles.adoc[Derived roles] that are assigned to to the principal by Cerbos while evaluating the policy. This is only populated in conditions in resource policies, and will only include derived roles that are referenced in at least one policy rule.
<5> Free-form context data about the principal.
<6> The resource on which the principal is performing actions.
<7> Resource kind.
<8> ID of the resource instance.
<9> Free-form context data about the resource instance.
<10> xref:configuration:auxdata.adoc[Auxiliary data sources].
<11> JWT claims.
...
To avoid duplication in condition expressions, you can define xref:variables.adoc[variables in policies].
