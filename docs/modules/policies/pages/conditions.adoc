...
| exists   | Check whether at least one element matching the predicate exists in a list or map. Support two variable form: <list\|map>.exists(<indexVar\|keyVar>, <valueVar>, <predicate>) | P.attr.teams.exists(t, t.startsWith("comm"))
| exists_one | Check that only one element matching the predicate exists. Support two variable form: <list\|map>.exists_one(<indexVar\|keyVar>, <valueVar>, <predicate>) | P.attr.teams.exists_one(t, t.startsWith("comm")) == false
| filter   | Filter a list using the predicate. Support two variable form: <list\|map>.filter(<indexVar\|keyVar>, <valueVar>, <predicate>)| size(P.attr.teams.filter(t, t.matches("^comm"))) == 2
| flatten   | Flattens a list. If an optional depth is provided, the list is flattened to a the specified level. A negative depth value will result in an error| 1,2,[],[],[3,4]].flatten() == [1, 2, 3, 4] && [1,[2,[3,[4]]]].flatten(2) == [1, 2, 3, [4]]
...
| range    | Returns a list of integers from 0 to n-1 | lists.range(5) == [0, 1, 2, 3, 4]
...
| sort     | Sorts a list with comparable elements. If the element type is not comparable or the element types are not the same, the function will produce an error | [3, 2, 1].sort() == [1, 2, 3]
