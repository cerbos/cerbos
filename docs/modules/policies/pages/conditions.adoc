...
| exists   | Check whether at least one element matching the predicate exists in a list or map. | P.attr.teams.exists(t, t.startsWith("comm")) && P.attr.limits.exists(k, v, size(k) > 3 && v > 0)
| exists_one | Check that only one element matching the predicate exists. | P.attr.teams.exists_one(t, t.startsWith("comm")) == false && P.attr.limits.exists_one(k, v, size(k) > 3 && v > 0) == false
...
| transformList | Converts a map or a list into a list value. The output expression determines the contents of the output list. Elements in the list may optionally be filtered | [1, 2, 3].transformList(i, v, i % 2 == 0, (i * v) + v) == [1, 9] && +
[1, 2, 3].transformList(i, v, (i * v) + v) == [1, 4, 9]
| transformMap | Converts a map or a list into a map value. The output expression determines the value of the output map entry; however, the key remains fixed. Elements in the map may optionally be filtered| [1, 2, 3].transformMap(i, v, i % 2 == 0, (i * v) + v) == {0: 1, 1: 9}
