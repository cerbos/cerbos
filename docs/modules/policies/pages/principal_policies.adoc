        ...
        - name: dev_record_wildcard <7>
          ...
          output: <10>
            when:
              ...
              ruleNotActivated: |-
                "wildcard_rule_not_activated:%s".format([request.principal.id])
...
depending on whether the rule activates or not. An output expression can contain anything that condition expressions can have.
