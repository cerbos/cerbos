...
You can define an optional expression to be evaluated when a policy rule is activated or not activated.
...
NOTE: The expression stated in the `output.when.condFail` will be executed if the condition in the relevant rule is failed.
