...
If you work in an AI coding agent, the xref:agent_skill.adoc[Cerbos agent skills] give it the full policy reference and validate the output against the Cerbos PDP.
...
[TIP]
====
For a review of policy design against a specific use case, https://www.cerbos.dev/workshop?utm_campaign=brand_cerbos&utm_source=documentation&utm_medium=text&utm_content=authoring_tips[book a policy review session with Cerbos].
====
