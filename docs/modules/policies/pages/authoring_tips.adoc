...
* The JSON schema for Cerbos policies is available at `https://api.cerbos.dev/v{app-version}/cerbos/policy/v1/Policy.schema.json`. If you prefer to always use the latest version, it can be accessed at `https://api.cerbos.dev/latest/cerbos/policy/v1/Policy.schema.json` as well. 
