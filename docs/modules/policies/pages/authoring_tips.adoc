...
Go into the Preferences of JetBrains IDE of choice and navigate to the `JSON Schema Mappings` menu.
...
Before starting, make sure you know how to modify Visual Studio Code settings by taking a look at the link:https://code.visualstudio.com/docs/getstarted/settings[VSCode documentation].

For Visual Studio Code, an extension is required to work with the schema.
Please install the extension from link:https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml[here]

After the required extension is ready, please navigate to the Visual Studio Code's `settings.json` and add the following part.
