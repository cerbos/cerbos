...
<6> List of outcome expected for a specified principal and a given action.

It is possible to share principals and resources between
test suites stored in the same directory. Create a `testdata` directory in the directory with your test suite files, then put shared resources and principals to `testdata\resources.yml` and `testdata\principals.yml` respectively (`yaml` and `json` extensions are also supported).
