...
tests: <14>
  - name: Accessing an album <15>
    options: <16>
      now: "2022-08-03T15:00:00Z" <17>
      defaultPolicyVersion: production <18>
      defaultScope: "" <19>
      lenientScopeSearch: false <20>
      strictEvaluation: true <21>
      globals: <22>
        my_global_var: bar
...
<7> Optionally enable xref:configuration:engine.adoc#strict_evaluation[strict evaluation] for this test suite.
<8> Optionally set xref:configuration:engine.adoc#globals[globals] for this test suite.
<9> Map of principal fixtures. The key is a string that can be used to refer to the associated principal.
<10> Map of principal groups. The key is a string that can be used to refer to the associated group of principal fixtures.
<11> Map of resource fixtures. The key is a string that can be used to refer to the associated resource.
<12> Map of resource groups. The key is a string that can be used to refer to the associated group of resource fixtures.
<13> Map of (optional) auxiliary data fixtures required to evaluate some requests. The key is a string that can be used to refer to the associated auxData.
<14> List of tests in this suite.
<15> Name of the test.
<16> Optionally set options that apply to just this test. Test-specific options are not merged with suite-wide options, so any unspecified values revert to the default.
<17> Optional RFC3339 timestamp to be used as the return value of the `now` function.
<18> Optionally set xref:configuration:engine.adoc#default_policy_version[default policy version] for this test.
<19> Optionally set xref:configuration:engine.adoc#default_scope[default scope] for this test suite.
<20> Optionally set xref:configuration:engine.adoc#lenient_scopes[lenient scope search] for this test.
<21> Optionally enable xref:configuration:engine.adoc#strict_evaluation[strict evaluation] for this test.
<22> Optionally set xref:configuration:engine.adoc#globals[globals] for this test.
<23> Input to the policy engine.
<24> List of keys of principal fixtures to test.
<25> List of keys of resource fixtures to test.
<26> List of actions to test.
<27> Key of auxiliary data fixture to test (optional).
<28> List of outcomes expected for each principal and resource. If a principal+resource pair specified in `input` is not listed in `expected`, then `EFFECT_DENY` is expected for all actions for that pair.
<29> Key of the principal fixture under test. Use `principals` instead of `principal` if you want to specify identical expectations for multiple principals.
<30> Key of the resource fixture under test. Use `resources` instead of `resource` if you want to specify identical expectations for multiple resources.
<31> Expected outcomes for each action for each principal+resource pair. If an action specified in `input` is not listed, then `EFFECT_DENY` is expected for that action.
<32> Optional list of xref:outputs.adoc[output values] to match.
<33> Name of the action that would produce the output.
<34> List of expected output values.
<35> List of keys of principal groups to test. You can provide this instead of, or as well as, `principals`.
<36> List of keys of resource groups to test. You can provide this instead of, or as well as, `resources`.
<37> Key of the principal group under test. You can provide this instead of, or as well as, `principal` or `principals`.
<38> Key of the resource group under test. You can provide this instead of, or as well as, `resource` or `resources`.
