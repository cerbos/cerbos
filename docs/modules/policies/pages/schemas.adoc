...
NOTE: The `schemas` field is optional and can be left out if you do not want to validate that particular resource kind. Alternatively, you can leave out the `principalSchema` or the `resourceSchema` if you do not want the corresponding attributes section to be validated.
