...
func decryptBundleV2(opts OpenOpts, logger *zap.Logger) (string, int64, error) {
	...
	if err != nil {
		logger.Debug("Failed to read bundle v2", zap.Error(err))
		return "", 0, fmt.Errorf("failed to read bundle v2 at path %q: %w", opts.BundlePath, err)
	}
	...
	if opts.Credentials == nil {
		...
		logger.Debug("Decrypting bundle")
		if decrypted, err = opts.Credentials.DecryptV2(b); err != nil {
			logger.Debug("Failed to decrypt bundle v2", zap.Error(err))
			return "", 0, fmt.Errorf("failed to decrypt bundle v2: %w", err)
		}
	}
	...
	size, err := io.Copy(outFile, bytes.NewBuffer(decrypted))
	if err != nil {
