...
func TestCerbosLib(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.expr, func(t *testing.T) {
			...
			have, _, err := conditions.Eval(env, ast.NativeRep(), cel.NoVars(), conditions.Now())
			if tc.wantErr {
				is.Error(err)
			} else {
				is.NoError(err)
...
func TestPartialEvaluationWithMacroGlobalVars(t *testing.T) {
	...
	out, det, err := conditions.Eval(env, ast.NativeRep(), vars, time.Now, cel.EvalOptions(cel.OptTrackState, cel.OptPartialEval))
	is.NoError(err)
