...
func TestQueryResourceKindForms(t *testing.T) {
	// Regression test for https://github.com/cerbos/cerbos/issues/3388
	const (
		rawKind       = "udm:module:users/simple_regular-user"
		sanitizedKind = "udm_module_users_simple_regular_user"
	)
	require.Equal(t, sanitizedKind, namer.SanitizedResource(rawKind))

	impl := index.New()
	require.NoError(t, impl.IndexRules([]*runtimev1.RuleTable_RuleRow{
		// Resource policy rows are indexed under the sanitized kind.
		makeRow(namer.ResourcePolicyFQN(rawKind, "default", ""), func(r *runtimev1.RuleTable_RuleRow) {
			r.Resource = sanitizedKind
			r.FromRolePolicy = false
			r.Name = "resource_policy"
		}),
		// Role policy rows keep the kind as written.
		makeRow(namer.RolePolicyFQN("admin", "default", ""), func(r *runtimev1.RuleTable_RuleRow) {
			r.Resource = rawKind
			r.Role = "admin"
			r.Name = "role_policy_literal"
		}),
		makeRow(namer.RolePolicyFQN("admin", "default", ""), func(r *runtimev1.RuleTable_RuleRow) {
			r.Resource = "udm:module:users/*"
			r.Role = "admin"
			r.Name = "role_policy_glob"
		}),
		// Globs are matched against the kind as written only, so this must not match rawKind.
		makeRow(namer.RolePolicyFQN("admin", "default", ""), func(r *runtimev1.RuleTable_RuleRow) {
			r.Resource = "udm_module_*"
			r.Role = "admin"
			r.Name = "sanitized_form_glob"
		}),
		// Principal policy rows store literal resources in sanitized form and globs as written.
		makeRow(namer.PrincipalPolicyFQN("alice", "default", ""), func(r *runtimev1.RuleTable_RuleRow) {
			r.PolicyKind = policyv1.Kind_KIND_PRINCIPAL
			r.Principal = "alice"
			r.Role = "*"
			r.FromRolePolicy = false
			r.Resource = sanitizedKind
			r.Name = "principal_policy_literal"
		}),
		makeRow(namer.PrincipalPolicyFQN("alice", "default", ""), func(r *runtimev1.RuleTable_RuleRow) {
			r.PolicyKind = policyv1.Kind_KIND_PRINCIPAL
			r.Principal = "alice"
	...
	t.Run("principal policy rows only", func(t *testing.T) {
		res := impl.Query("default", rawKind, "", "", nil, policyv1.Kind_KIND_PRINCIPAL, "alice", nil)
		require.ElementsMatch(t, []string{"principal_policy_literal", "principal_policy_glob"}, names(res))
	})

	t.Run("unrelated kind", func(t *testing.T) {
		require.Empty(t, impl.Query("default", "udm:module:groups/simple_group", "", "", nil, 0, "", nil))
		require.False(t, impl.ScopedResourceExists("default", "udm:module:groups/simple_group", []string{""}))
	})
}
